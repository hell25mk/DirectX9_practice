...
#pragma region 細かい画面モードの設定
	...
	if(FAILED(d3dObject.pD3D->CreateDevice(
		...
		windowObject.hWnd,							//表示するウィンドウ（CreateWindowで取得したハンドル）
...
/// <summary>
/// 確保したDirectX関連メモリを解放する
/// </summary>
/// <returns>true:成功 false:失敗</returns>
bool DirectX::DxEnd(){
	
	ReleaseObject(d3dObject.pSprite);
	ReleaseObject(d3dObject.pD3Ddevice);
	ReleaseObject(d3dObject.pD3D);
